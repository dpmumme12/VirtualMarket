      ...
      if (response_status === 201) {
        if (TransactionType === "BUY") {
          ...
          document.getElementById(
            "alert-text"
          ).innerHTML = `<i class="bi bi-check-circle-fill" style="padding-right: 10px;"></i> ${shares} shares of ${quote.symbol} purchased successfully.`;
          ...
          document.getElementById(
            "alert-text"
          ).innerHTML = `<i class="bi bi-check-circle-fill" style="padding-right: 10px;"></i> ${shares} shares of ${quote.symbol} sold successfully.`;
          document.getElementById("alert-wrapper").style.display = "block";
        }
        ...
        document.getElementById(
          "alert-text"
        ).innerHTML = `<i class="bi bi-exclamation-triangle-fill" style="padding-right: 10px;"></i> ${resp.data.msg}`;
        document.getElementById("alert-wrapper").style.display = "block";
      } else {
        document.getElementById("alert-wrapper").className =
          "alert alert-danger alert-dismissible";
        document.getElementById("alert-text").innerHTML =
          '<i class="bi bi-exclamation-triangle-fill" style="padding-right: 10px;"></i> Oops something has gone wrong.';
        document.getElementById("alert-wrapper").style.display = "block";
      }
    });
...
var socket = new WebSocket(
  `${ws_scheme}://${window.location.host}/ws/stock/${symbol}/`
);
...
function StockSocketMessage(elem) {
  ...
  document.getElementById("CurrentPrice").innerHTML =
    quote.latestPrice.toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
    });
}
